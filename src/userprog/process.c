...
process_exit (void)
{
  ...
  for (e = list_begin (&cur->children); e != list_end (&cur->children);
    ...
    hash_destroy(&thread_current()->suppl_page_table, page_free);
...
load (const char *cmd_line, void (**eip) (void), void **esp) 
{
  ...
  hash_init (&thread_current()->suppl_page_table,page_hashing,page_less,NULL);
          ...
          if (validate_segment (&phdr, file)) 
            {
              ...
              if (phdr.p_filesz > 0)
                ...
                //bool out = hash_init (&thread_current()->suppl_page_table,page_hashing,page_less,NULL);
              ...
              uint32_t read_bytes, uint32_t zero_bytes, bool writable) 
{
  ...
  spt = &thread_current()->suppl_page_table;
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      /* Get a page of memory from user pool frame table*/
      ...
      bool pageadd_success = pagetable_addfile(file,page_zero_bytes,page_read_bytes,writable,ofs,upage,spt);
      if(!pageadd_success)
        return false;
      /* Advance. */
...
setup_stack (const char *cmd_line, void **esp) 
{
  uint8_t *kpage;
  bool success = false;
  //success = add_stack(((uint8_t *)PHYS_BASE) - PGSIZE);
  /*if(success){
    *esp = PHY_BASE;
  }*/
  /*else
  ...
  if(spt_entry){
    ...
    spt_entry->user_virtual_addr = pg_round_down(((uint8_t *) PHYS_BASE) - PGSIZE);
    spt_entry->page_accessed = true;
    spt_entry->is_writable = true;
    uint8_t *kpage = frame_allocate(PAL_USER,spt_entry);*/
  //kpage = palloc_get_page (PAL_USER | PAL_ZERO);
  struct suppl_page_table_entry *spt_entry = malloc(sizeof(struct suppl_page_table_entry));
  if(spt_entry){
    spt_entry->page_isloaded = true;
    spt_entry->user_virtual_addr = ((uint8_t *) PHYS_BASE) - PGSIZE;
    spt_entry->page_accessed = true;
    spt_entry->is_writable = true;
    ...
    if (kpage != NULL) 
      {
        ...
        if (install_page (upage, kpage, true)){
          struct hash_elem *success1 = hash_insert(&thread_current()->suppl_page_table, &spt_entry->hash_table_elem);
          if(success1 == NULL)
            success = init_cmd_line (kpage, upage, cmd_line, esp);
          else
            return false;
          //palloc_free_page(cmd_line);
          //hex_dump(0,*esp,150,true);
        }
        else
          palloc_free_page(kpage);
      }
...
install_page (void *upage, void *kpage, bool writable)
{
  ...
  if(temp1 && pagedir_set_page (t->pagedir, upage, kpage, writable)) {
    temp2 = true;
  }
  return temp1 && temp2;
